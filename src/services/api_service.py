...
def create_ticket(state=None, ticket_description=None, doctor_name=None, patient_name=None):
    """
    ...
    try:
        ...
        graph_state = (state or {}).get("graph_state", {})
        print("Creating ticket with state:", graph_state)
        organisation_id = (graph_state.get("organisation_details") or {}).get("organisation_id") or {}
        print("Extracted organisation_id for ticket creation:", organisation_id)
        patient_phone_no = graph_state.get("sender")
        print("Extracted patient_phone_no for ticket creation:", patient_phone_no)
        message_history=(state or {}).get("messages", [])
        print(f"Extracted message history for ticket creation: {len(message_history)} messages")
        ticket_description = ticket_description or "User asked to talk to human agent, creating ticket for follow-up."
...
def create_ticket_API(ticket_description=None):
    """
    Create a support ticket using Supabase Edge Function.
    Args:
        state (dict): State containing organisation_id and patient phone number
    ...
    try:
        ...
        from src.services.create_ticket_service import create_ticket_natively_API
        data = create_ticket_natively_API(
            ticket_description=ticket_description,
        )
        if data.get("result_code") == 101:
            print("✅ Ticket created successfully natively:", data)
    ...
    try:
        ...
        if data.get("result_code") == 101:
            ...
            if code == 500:
                ...
                error_description = (
                    f"[API ERROR] Cancel appointment API failed. Result code 500\n"
                    f"error: {str(err)}\n"
                    )
                print("Creating ticket for cancellation error...")
                create_ticket_API(
        ...
        error_description = (
                f"[API ERROR] cancel appointment API failed.\n"
                f"error: {str(e)}\n"
                )
        print("Creating ticket for cancellation exception...")
        create_ticket(
                state=state,
                ticket_description=error_description
            )
    ...
    try:
        ...
        if result_code == 101:
            ...
            if result_code == 500:
                ...
                error_description = (
                    f"[API ERROR] reschedule appointment API failed. Result code 500\n"
                    f"error: {str(err)}\n"
                    )
                print("Creating ticket for rescheduling error...")
                create_ticket_API(
                    ticket_description=error_description
                )
        ...
        error_description = (
                f"[API ERROR] Reschedule appointment API failed.\n"
                f"error: {str(e)}\n"
    ...
    try:
        ...
        if result_code == 101:
            ...
            if result_code == 500:
                ...
                error_description = (
                    f"[API ERROR] Booking appointment API failed. Result code 500\n"
                    f"error: {str(err)}\n"
                    )
                print("Creating ticket for booking error...")
                create_ticket_API(
                    ticket_description=error_description
                )
            elif result_code == 409:
        ...
        error_description = (
                f"[API ERROR] Book appointment API failed.\n"
                f"error: {str(e)}\n"
                f"User message: {user_message}"
                )
