...
from src.services.message_logging_service import create_conversation, log_message
...
SERVICE_ACCOUNT_FILE = os.path.join(os.path.dirname(__file__), "../../medisync-bot.json")

def fetch_sheet(docId: str):
    """
...
def reply_node(state: graphState):
    ...
    if graph_state['next_node'] == "general_inquiry":
        ...
        if booking_done:
            ...
            if "doctor_agent" in agent_output:
                ...
                # Check if a flow was sent — if so, skip sending a text reply since the flow interactive message was already delivered via WhatsApp API
                patient_agent_type = agent_output.get("patient_agent", {}).get("type", "")
                if patient_agent_type == "flow_sent":
                    print("📋 Patient onboarding flow already sent — skipping text reply")
                    return state
    ...
    if conversation_id:
        ...
        log_message(conversation_id, "agent", data, agent_name=node_name)
...
def clear_state_memory(state):
    ...
    try:
        ...
        if composite_id:
            redis_key = f"conversation:{composite_id}"
            ...
            print(f"🧹 Conversation cleared for {composite_id}")
    except Exception as e:
...
def get_langgraph_response(message: str, sender: str, agent_phonenumber: str = None,phone_number_id: str = "", flow_data: dict = None):
    ...
    try:
        # Block reprocessing of the terminal message after a session just completed
        # if redis_client.exists(f"session_ended:{composite_id}"):
        #     print(f"🛑 session_ended marker active for {composite_id} — skipping to avoid reprocessing terminal message")
        #     return "skipped"

        # Resolve the correctly formatted agent phonenumber for DB constraints
        db_agent_phonenumber = resolve_db_agent_phonenumber(phone_number_id) or agent_phonenumber
        
        # Check if conversation_id exists in Redis for this sender+clinic
        cached_conversation_id = redis_client.get(redis_key)
        if cached_conversation_id:
            conversation_id = cached_conversation_id
            print(f"✅ Conversation ongoing, reusing conversation_id: {conversation_id}")
        else:
            # No ongoing conversation, create new one
            print("🆕 New conversation started")
            conversation_id = create_conversation(sender, db_agent_phonenumber)
            # Store in Redis (no expiration - will be deleted when state clears)
            if conversation_id:
                redis_client.set(redis_key, conversation_id,ex=86400)
    except Exception as e:
        print(f"❌ Redis error: {str(e)}")
        # Fallback: create new conversation if Redis fails
        db_agent_phonenumber = resolve_db_agent_phonenumber(phone_number_id) or agent_phonenumber
        conversation_id = create_conversation(sender, db_agent_phonenumber)
    ...
    # Detect flow response messages and flag them for direct routing
    is_flow_response = message.startswith("__FLOW_RESPONSE__") if message else False
    ...
    state = {
        "graph_state": {
            ...
            "is_flow_response": is_flow_response,
            "flow_response_data": flow_data  # Patient onboarding flow data (if any)
        },
    ...
    try:
        ...
        if conversation_id:
            ...
            log_message(conversation_id, "agent", "An error occurred during processing", error=detailed_error)
