...
def get_waba_id(organisation_id: str) -> str:
    ##Fetch the WABA ID for an organisation.
    response = supabase.table("organisation_whatsapp_integration") \
        .select("waba_id") \
        .eq("organisation_id", organisation_id) \
        .execute()

    if not response.data:
        raise ValueError(f"No WABA ID found for organisation_id: {organisation_id}")
    return response.data[0]["waba_id"]

def fetch_template_info_from_db(template_name: str, organisation_id: str):
    """Fetch template structure from our own DB."""
    try:
        response = supabase.table("whatsapp_template") \
            .select("template_name, variable") \
        ...
        raw_vars = template.get("variable")

        # Parse param names from the variable JSON column
        params = []
        if raw_vars:
        ...
        print(f"  Error fetching template from DB: {e}")
        return None
...
def send_whatsapp_template(to: str, template_name: str, variables: list, state: dict):
    """
    Sends a WhatsApp template message using Meta API, allowing dynamic variables.
    variables should be a list of dicts, e.g., [{"type": "text", "text": "Jay"}]
    """
    WHATSAPP_ACCESS_TOKEN, PHONE_NUMBER_ID = get_whatsapp_config(state)
    print(f"Using WhatsApp config for template - Access Token: {WHATSAPP_ACCESS_TOKEN[:10]}..., Phone Number ID: {PHONE_NUMBER_ID}")
